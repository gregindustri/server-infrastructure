...
extract_zip_into() {
	...
	# install-pty buffer, and if Wings's log consumer ever stalls (websocket
	# blip, etc) unzip blocks forever on write(). Redirecting to a log file
	# isolates us from that. On failure the log is dumped to stderr so the
